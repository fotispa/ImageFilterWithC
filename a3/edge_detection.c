...
#include "bitmap.h"
/*
 * Edge detection filter function. Which gets 8 pixels surrounding
 * each pixels and use those 9 pixels to get the pixel from the
 * provided function apply_edge_detection_kernel.
 */
void edge_detection_filter(Bitmap *bmp) {
    ...
    for (int i=0; i < bmp->height; i++) {
        for (int j=0; j < bmp->width; j++) {
            ...
            Pixel result = apply_edge_detection_kernel(three[0], three[1], three[2]);
