 ...
 */
Bitmap *read_header() {
    unsigned char first_ten[BMP_HEADER_SIZE_OFFSET];
    if (fread(first_ten, 1, BMP_HEADER_SIZE_OFFSET, stdin) != BMP_HEADER_SIZE_OFFSET){
        perror("File is wrong");
        exit(1);
    }
    Bitmap *head = malloc(sizeof(Bitmap));
    if (fread(&(head->headerSize), sizeof(int), 1, stdin) != 1){
        perror("File is wrong");
        exit(1);
    }
    head->header = malloc(head->headerSize);
    //reading 4 more bytes
    unsigned char remain[4];
    if (fread(remain, sizeof(unsigned char), 4, stdin) != 4) {
        perror("File is wrong");
        exit(1);
    }
    
    if (fread(&(head->width), sizeof(int), 1, stdin) != 1) {
        perror("File is wrong");
        exit(1);
    }
    if (fread(&(head->height), sizeof(int), 1, stdin) != 1) {
        perror("File is wrong");
        exit(1);
    }
    int remaining = head->headerSize - BMP_HEIGHT_OFFSET - 4;
    unsigned char remainder[remaining];
    if (fread(remainder, 1, remaining, stdin) != remaining) {
        perror("File is wrong");
        exit(1);
    }

    memcpy(head->header, first_ten, sizeof(first_ten));
    memcpy(head->header + BMP_HEADER_SIZE_OFFSET, &(head->headerSize), sizeof(int));
    memcpy(head->header + BMP_HEADER_SIZE_OFFSET + 4, remain, sizeof(remain));
    memcpy(head->header + BMP_WIDTH_OFFSET, &(head->width), sizeof(int));
    memcpy(head->header + BMP_HEIGHT_OFFSET, &(head->height), sizeof(int));
    memcpy(head->header + BMP_HEIGHT_OFFSET + 4, remainder, sizeof(remainder));
...
void scale(Bitmap *bmp, int scale_factor) {
    ...
    int file_size = sizeof(Pixel)*new_width*new_height + bmp->headerSize;
